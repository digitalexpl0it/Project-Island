...
- **Pack version** **`1.3.0`** (from **`gradle.properties`** **`mod_version`**); built **`manifest.json`** **`version`** matches. **CurseForge:** bump the **Project Island** mod **`fileID`** in **`manifest.json`** when you publish **`projectisland-1.3.0.jar`**.
