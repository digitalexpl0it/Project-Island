...
**Current focus:** **Phase 2 is done**; **Phase 4** starter + dock/link + rope topology + rope tiers are in. **Player progression UI:** **FTB Quests** + **ProgressiveStages** starter pack under **`examples/dev-progression/`** (copy into local **`run-client`** / **`run-server`** `config/` for dev). **Void rescue** teleports to **procedural island centers** first (same anchor as starter/HUD) to avoid rim loops. Roadmap detail: [TODO.md](TODO.md).
