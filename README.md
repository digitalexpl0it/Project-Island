...
- [MOD_LIST.md](MOD_LIST.md) — **required third-party JARs** for the official modpack (pinned filenames; install into your local **`run-client/mods`** / **`run-server/mods`** for Gradle dev runs — those trees are **gitignored**).

**Current focus:** **Phase 2 is done**; **Phase 4** includes starter + claims + rope topology/tiers, **rope surfing** along linked anchors, **linked-anchor mining** (damages rope HP), **larger islands** (`floatingIslandHorizontalRadiusBonus`), and **optional ore thinning** (`floatingIslandsOreMultiplier*`). **Player progression UI:** **FTB Quests** + **ProgressiveStages** under **`examples/dev-progression/`** (copy into **`run-client`** / **`run-server`** `config/` for dev). **Void rescue** prefers **procedural island centers** and skips ticks while **rope surfing**. Roadmap: [TODO.md](TODO.md).
...
**Dev runs:** a starter quest chapter and Project Island–specific stage files live under `examples/dev-progression/`. Copy them into your local **`run-client/`** / **`run-server/`** `config/` trees when needed (those game dirs are **gitignored** — worlds, logs, and downloaded JARs stay out of the repo). **Quest/chapter files** are picked up on the next start (or your FTB Quests reload path if you use it); use **`/progressivestages reload`** after editing stage TOML (and **`/progressivestages validate`** to catch typos). Copy **`examples/dev-progression/ProgressiveStages/triggers.toml`** into **`config/ProgressiveStages/`** with the **`pi_*`** stages — it maps **`minecraft:story/mine_stone` → `stone_age`** and **`minecraft:story/smelt_iron` → `iron_age`** so stock **`iron_age.toml`** ingredient locks line up with vanilla progression (and **JEI** shows the **Harpoon Gun** recipe after smelting iron). The harpoon shaped recipe lives in **`data/projectisland/recipe/harpoon_gun.json`**.
