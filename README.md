...
- **Debug (cheats / OP level 2):** `/projectisland island here` — prints the island key and `IslandRecord` state at your feet (void columns report none).
- **Nearby island HUD (read-only, 2026-04-22):** while you are in the **floating-islands overworld** (server detects `FloatingIslandsChunkGenerator`), the server periodically sends labels for islands in a configurable region radius around you (availability, claimed owner name when known). The client draws a **billboard panel** above each island when you are in **`minecraft:overworld`** (see-through / night boost in client config). **64×64 PNG icons** in `assets/projectisland/textures/gui/island_hud/` — claimed uses `floating-island.png`; available slowly alternates `floating-island_ex.png` / `floating-island.png` (~1.25 s per frame at 20 TPS) so it reads as a pulse, not a strobe; contested uses a vanilla torch item. **Procedural two-word display names** are deterministic from the region id, plus status and `regionX;regionZ` id line. Client-side, labels are skipped when the beacon’s chunk is not loaded or the anchor is beyond ~90% of **render distance** (so they do not hang in empty sky past where terrain usually draws). Tuning and night readability live in **common** and **client** config (see worldgen tuning bullet above). This is **not** a claim UI or world border—those remain future work (see [TODO.md](TODO.md) Phase 4).
...
_Documentation last revised **24 April 2026** (Phase 2 overworld + `multi_noise`, spawn pregen, README migration note)._
