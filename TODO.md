...
- [ ] **Secondary claims — dock / link model:** design spec (distance, facing, blocks/entities, anti-exploit); server validation path; then implementation after Phase 2 + minimal movement MVP (or defer full rules until Phase 6). **MVP progress:** harpoon max span + raycast are **common config** (`ropeLinkMaxLengthBlocks`, `ropeLinkRaycastRangeBlocks`); `RopeLink.maxLengthBlocks` stores the cap for future tension / dock rules. **`/projectisland island claim`** (OP) requires an owned **rope link** to the target when `secondaryClaimRequiresRopeLink` is true (see `FloatingIslandSavedData#hasRopeLinkFromClaimedIsland`).
- [ ] **Rope stress + tiers (server):** per-link **health / tension / strain** (and caps from **per-player** progression and item tier — upgrades apply automatically to new or existing links per chosen rules). When a link snaps or is removed, **revalidate** extra-island claims: lose claim on an island if no qualifying link path remains (**starter** exempt). **No team-shared unlock tiers** for v1 (alliance claim access is separate from progression; see Phase 5 invite/team item).
