...
- [x] **New player → region-spiral `AVAILABLE` + atomic auto-claim + center teleport:** `FloatingIslandStarterPlacement` on **`PlayerLoggedIn`** (before void rescue); **`FloatingIslandSavedData`** `StarterHomes` map + **`tryClaimStarterIsland`**; spiral from **world spawn** (or join chunk); **center / HUD** teleport via `regionIsland` + `islandSurfaceBlockY`; config `starterIsland*`; optional **chunk ticket** before teleport not implemented yet.
