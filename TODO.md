...
- [x] **Secondary claims — dock / link model:** formal spec **[docs/phase4-dock-link-spec.md](docs/phase4-dock-link-spec.md)** — harpoon physics, `RopeTopology`, `hasRopeLinkFromClaimedIsland`, command vs sneak-anchor surfaces, revalidation on rope break, anti-exploit gaps, optional hardening (proximity-at-command, dock tags). **MVP in code:** `ropeLinkMaxLengthBlocks` / `ropeLinkRaycastRangeBlocks`, `RopeLink.maxLengthBlocks`, `/projectisland island claim` + anchor sneak-use, `secondaryClaimRequiresRopeLink`.
...
- [x] **Interim command + anchor claim:** superseded by permission **`secondaryClaimCommandPermissionLevel`** and rope-anchor interaction; distance / facing / dock tags — see **[docs/phase4-dock-link-spec.md](docs/phase4-dock-link-spec.md)** §7–9.
