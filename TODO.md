...
- **Phase 2 is closed** (overworld floating islands, `multi_noise`, optional spawn pregen). **Phase 4** claims + rope topology are in; **player-facing progression** uses an **FTB Quests + ProgressiveStages** baseline in-repo (see `examples/dev-progression/`). Next: tighten **server-side stage gating** for Project Island rules, and optional **quest tasks** for first rope link / secondary claim (replace manual checkmark).
...
- [x] **Rope stress + tiers (server):** per-link **health / tension / strain**. **Done:** health + strain snap; **advancement-driven rope tiers** (`RopeProgression`: `projectisland:progression/rope_reinforced` / `rope_steel` scale max length + max health); **existing links auto-upgrade** on a server interval (`RopeLinkProgressionUpgrade`, preserves health fraction; `ropeProgressionUpgradeExistingLinks` / `ropeProgressionUpgradeIntervalTicks`); **`removeRopeLink`** → **`revalidateRopeBackedClaimsForOwner`** when **`secondaryClaimRequiresRopeLink`**. **Still open:** extra tiered caps beyond length/health, item-tier hooks beyond advancements if desired. **No team-shared unlock tiers** for v1 (alliance claim access is separate from progression; see Phase 5 invite/team item).
