...
- [ ] **Island HUD polish (optional):** full **RGB color picker** for panel/border; optional **datapack-driven** icon layout (would still need client assets or a resource pack for bitmaps). **Done in-repo:** JSON **`adjectives` / `nouns`** via `data/projectisland/floating_island_display_names/names.json` + **`/reload`**; two HUD sprites **`floating-island.png`** / **`floating-island_ex.png`** + vanilla torch for contested; example override pack **`examples/island_hud_icons_resource_pack/`**; **`islandHudPanelScale`** + **`islandHudPanelFillOpacity`** / **`islandHudPanelBorderOpacity`** in `projectisland-client.toml`.
...
- [ ] **Secondary claims — dock / link model:** design spec (distance, facing, blocks/entities, anti-exploit); server validation path; then implementation after Phase 2 + minimal movement MVP (or defer full rules until Phase 6). **MVP progress:** harpoon max span + raycast are **common config** (`ropeLinkMaxLengthBlocks`, `ropeLinkRaycastRangeBlocks`); `RopeLink.maxLengthBlocks` stores the cap for future tension / dock rules. **`/projectisland island claim`** requires an owned **rope link** to the target when `secondaryClaimRequiresRopeLink` is true (see `FloatingIslandSavedData#hasRopeLinkFromClaimedIsland`).
- [x] **Rope topology enforcement (server):** **`RopeTopology`** before the second harpoon anchor — BFS depth from starter, **`ropeAllowTertiaryIslandLinks`** (default **false** caps at one hop; **true** uses **`ropeTopologyMaxDepthFromStarter`**, e.g. **2** for tertiary), **main direct spoke** cap (default **1**, up to **4** in config), **sister outbound** cap per claimed non-starter (default **1**, up to **3**). **`ropeTopologyEnabled`** to disable. **Advancement-driven** tertiary unlock still future (toggle stands in).
- [ ] **Rope stress + tiers (server):** per-link **health / tension / strain** (and caps from **per-player** progression and item tier — upgrades apply automatically to new or existing links per chosen rules). **MVP:** health + strain snap; **`removeRopeLink`** → **`revalidateRopeBackedClaimsForOwner`** when **`secondaryClaimRequiresRopeLink`**: non-starter **CLAIMED** islands without a **direct** owned rope to another claimed island revert to **AVAILABLE**. **Still open:** depth/spoke **enforcement** + advancement caps, item tiers. **No team-shared unlock tiers** for v1 (alliance claim access is separate from progression; see Phase 5 invite/team item).
...
- [x] **Claim** action (non-starter): **`IslandSecondaryClaim`** + **`trySecondaryClaim`**; **`/projectisland island claim`** (configurable permission, default **0**) **or** **sneak + use** on the claimer’s **linked rope anchor** on the target island. Rope-to-owned-island rule when `secondaryClaimRequiresRopeLink`. HUD refresh already exists.
- [x] **Interim command + anchor claim:** superseded by permission **`secondaryClaimCommandPermissionLevel`** and rope-anchor interaction (dock distance / facing still TBD).
...
- [ ] **Advancement / progress UI (optional, not a dep):** [Advancements Reloaded](https://github.com/42atomys/mc-advancements-reloaded) (MIT, NeoForge; [Modrinth](https://modrinth.com/mod/advancements-reloaded)) helps players **track** done vs remaining steps toward **new gear** and **upgrades** (search, ordering, layout on the vanilla advancements screen). It does **not** implement unlock logic. Pin a tested **1.21.x** build; note **Known issues** (another mod replacing the same screen). **Fork** only if a required UX cannot land upstream—otherwise contribute or add a small Project Island overlay. Heavier “quest journal” mods (e.g. FTB Quests–class) are a separate product from vanilla-style advancement trees.
